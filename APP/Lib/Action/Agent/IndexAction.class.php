...
public  function routelist(){
	...
	public function lianmeng(){
		...
		if(IS_POST){
			$lmurl = $_POST['lmurl'];
			$lmimgurl = $_POST['lmimgurl'];
			$dalm['lmurl'] = $lmurl;
			$dalm['lmimgurl'] = $lmimgurl;
			$ddd = $dbagent->where('id='.$aid)->save($dalm);
			if($ddd>0){
				$data['error']=0;
				return $this->ajaxReturn($data);
			}else{
				$data['error']=1;
				$data['msg']=$dbagent->getError();
				return $this->ajaxReturn($data);
			}
		}else{
			$agentinfo = $dbagent->where('id='.$aid)->find();
			$this->assign('info',$agentinfo);
		}
		$this->display();
	}
}