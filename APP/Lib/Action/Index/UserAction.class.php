...
class UserAction extends BaseAction{
	...
	public function mangeroute()
    {
        ...
        if(IS_POST){
        	...
        	$route= D('Routesite');
        	$_POST['shopid']=session('uid');
        	$routemap_id = I('post.routemap_id','0','int');
        	$_POST['encrypt'] = intval($_POST['encrypt']);
			
	        //$_POST['rzaite_yubai'] = json_encode(explode(',',$_POST['rzaite_yubai']));
	        $_POST['rzaite_yubai'] = '0';
	        $_POST['route_ip'] = '0';
	        $_POST['route_mask'] = '0';
	        $_POST['route_gateway'] = '0';
			$where['routemap_id']  = $routemap_id;
			...
			if($route->where($where)->save() !== false){
			...
			$site = D('Routesite')->where($data)->find();
    ...
    public function delrout()
    {
		...
		//$sql="delete m, s from routemap m inner join routesite s on m.id = s.route_ip where m.shopid="$uid";
		
        $r = D('Routemap')->where($where)->find();
        $site = D('Routesite')->where($data)->find();
		//var_dump(D('Routemap')->getlastsql());exit;
        if($r==false){
        	$this->error('没有此路由信息');
        }else{
          if(D('Routemap')->where($where)->delete()){
			if(D('Routesite')->where($data)->delete()){
				$this->success('删除成功');
			}
          }else{
          	$this->error('删除失败');
	...
	/**
	author   : written by charles
	date     : 2015/04/03
	function : actrpt and activity
	table    : wifi_useract
	**/
	...
	public function addactlog(){
		if(IS_POST){
	        ...
	        if(!$upload->upload()) {
				...
				//var_dump($info);exit;
				$catedb=D('Useract');
				//var_dump($Useract);exit;
				if($catedb->create()){
					if($catedb->add()){
						$this->success("添加成功",U('user/actrpt'));
					}else{
						$this->error("添加失败");
					}
				}else{
	...
	public function lianmengset(){
		$uid = $_SESSION['uid'];
		$dbshop = M('shop');
		if($this->isPost()){
			$lianmengurl = $_POST['lianmengurl'];
			$data['lianmengurl'] = $lianmengurl;
			$data['id'] = $uid;
			$result = $dbshop->save($data);
			if($result>0){
				$this->success('修改成功！');
			}
		}else{
			$shop = $dbshop->where('id='.$uid)->find();
			$this->assign('shop',$shop);
			$this->assign('a','lianmeng');
			$this->display();
		}
