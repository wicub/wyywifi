var DD_belatedPNG = {
	...
	createVmlStyleSheet : function() {
		...
		document.documentElement.firstChild.insertBefore(c,
	...
	fix : function(e) {
		if (this.screenStyleSheet) {
			...
			for (d = 0; d < f.length; d++) {
				this.screenStyleSheet.addRule(f[d],
	...
	vmlOffsets : function(x) {
		...
		w(x.vml.color.shape, (b.L + (x.isImg ? 0 : b.bLW)), (b.T + (x.isImg
						? 0
						: b.bTW)), (b.W - 1), (b.H - 1), 0);
		...
		if (c != "repeat" || x.isImg) {
			...
			x.vml.image.shape.style.clip = "rect(" + s.T + "px " + (s.R + y)
					+ "px " + s.B + "px " + (s.L + y) + "px)"
		} else {
			x.vml.image.shape.style.clip = "rect(" + v.T + "px " + v.R + "px "
					+ v.B + "px " + v.L + "px)"
		}
	},
